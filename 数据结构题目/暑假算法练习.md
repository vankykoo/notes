...
class Solution {
    public int minDistance(String word1, String word2) {
        ...
        // 初始化dp数组
        for (int i = 0; i <= m; i++) {
            dp[i][0] = i; // word2为空字符串时，把word1转换为word2所需的编辑距离
        }

        for (int j = 0; j <= n; j++) {
            dp[0][j] = j; // word1为空字符串时，把word2转换为word1所需的编辑距离
        }
        ...
        for (int i = 1; i <= m; i++) {
            for (int j = 1; j <= n; j++) {
                if (word1.charAt(i - 1) == word2.charAt(j - 1)) {
                    ...
                    dp[i][j] = Math.min(dp[i - 1][j - 1], // 替换操作
...
1. **候选人选择**：我们使用 `candidate` 变量来保存当前的多数元素候选人，并通过 `count` 变量来计数。

2. 遍历数组

   ：

   - 当 `count` 为 0 时，说明前面部分的数字已经无法确定多数元素，所以将 `candidate` 更新为当前的数字。
   - 如果当前数字等于 `candidate`，则计数 `count` 加 1，否则减 1。

3. **返回结果**：在遍历完数组后，`candidate` 即为多数元素。
...
1. **链表环检测**：
   - 假设数组中存在一个重复的数字，这意味着有多个索引指向相同的数字。这种情况下，将数组看作链表，会导致链表中出现环。
   - 例如，对于数组 `nums = [1, 3, 4, 2, 2]`，可以将其转换为链表：`1 -> 3 -> 2 -> 4 -> 2`，你会发现 `2` 会形成环。
2. **快慢指针**：
   - 使用两个指针 slow和 fast，起始时都指向数组的第一个元素。然后进行迭代：
     - `slow` 指针每次移动一步，即 `slow = nums[slow]`。
     - `fast` 指针每次移动两步，即 `fast = nums[nums[fast]]`。
   - 由于 `fast` 指针移动得比 `slow` 指针快，如果链表中存在环，那么 `slow` 和 `fast` 最终会在环中相遇。
3. **找到环的入口**：
   - 当 `slow` 和 `fast` 相遇后，将 `fast` 指针重新指向数组的起点，并且将 `fast` 和 `slow` 指针都以相同的速度（每次移动一步）前进。
   - 他们相遇的地方就是环的入口，也就是重复的数字所在的位置。
