...
class Solution {
    public int lengthOfLIS(int[] nums) {
        // 初始化返回结果为1，因为最短的递增子序列长度为1
        int res = 1;
        int len = nums.length;
        // dp数组用于存储以每个元素结尾的最长递增子序列的长度
        int[] dp = new int[len];
        // 初始化dp数组的第一个元素，单个元素本身长度为1
        dp[0] = 1;

        for(int i = 1; i < len; i++){
            // 每个元素最短的递增子序列长度为1（只包含自己）
            dp[i] = 1;
            // 对于当前元素nums[i]，比较之前所有的元素nums[j]
            for(int j = 0; j < i; j++){
                // 如果之前的元素小于当前元素，说明可以形成一个递增子序列
                if(nums[j] < nums[i]){
                    // 更新dp[i]的值为当前的dp[i]和dp[j]+1中的最大值
                    // dp[j] + 1表示在nums[j]后面添加nums[i]形成的新子序列长度
...
class Solution {
    public int longestCommonSubsequence(String text1, String text2) {
        ...
        // 初始化二维数组的第一列
        boolean tag = false; // 标记是否已经找到匹配字符
        for(int i = 0; i < len1; i++){
            // 如果找到匹配字符或者之前已经找到匹配字符，则将dp[i][0]设为1
            if(text1.charAt(i) == text2.charAt(0) || tag){
                tag = true; // 标记为已经找到匹配字符
                dp[i][0] = 1;
            }
        }
        ...
        for(int i = 0; i < len2; i++){
            // 如果找到匹配字符或者之前已经找到匹配字符，则将dp[0][i]设为1
            if(text1.charAt(0) == text2.charAt(i) || tag){
                tag = true; // 标记为已经找到匹配字符
                dp[0][i] = 1;
            }
        }
        ...
        for(int i = 1; i < len1; i++){
            for(int j = 1; j < len2; j++){
                // 如果当前字符匹配，则dp[i][j] = dp[i-1][j-1] + 1
                if(text1.charAt(i) == text2.charAt(j)){
                    dp[i][j] = dp[i-1][j-1] + 1;
                } else {
                    // 否则，dp[i][j]取dp[i-1][j]和dp[i][j-1]中的最大值
